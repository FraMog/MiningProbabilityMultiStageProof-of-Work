...
ComputeMiningProbability::usage = "ComputeMiningProbability[ { {\!\(\*SubscriptBox[\(\[Lambda]\), \(1, 0\)]\), ..., \!\(\*SubscriptBox[\(\[Lambda]\), \(1, k - 1\)]\)}, ..., 
{\!\(\*SubscriptBox[\(\[Lambda]\), \(M, 0\)]\), ..., \!\(\*SubscriptBox[\(\[Lambda]\), \(M, k - 1\)]\)} }] 
computes the mining probability of the first of M miners. Each one of the M miners is identified by a list of k positive real \[Lambda]-parameters. 
The function implicitly determines the values of M and k by parsing the input. It must hold that M>=2 and k>=1.";
